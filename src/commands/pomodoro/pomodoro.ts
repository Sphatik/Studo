...
const pomodoroSpeaker: IPomodoroSpeaker = {
  playStart() {
  ...
  playBreakComplete(_session: PomodoroSession | PomodoroSessionAttributes) {
    // TODO: Implement TTS for break completion
  },
};
...
export async function handleButtonInteraction(interaction: ButtonInteraction, _client: Client) {
...
async function handleStartCmd(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const _existingSession = await getActiveChannelPomodoro(channelId, voiceChannelId);
...
async function handleStatus(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const _embed = embedTimerStatus(session, 'Pomodoro Timer Status', false);
}
...
async function handleLeaderboard(_interaction: ChatInputCommandInteraction): Promise<void> {
...
async function _completeBreak(session: PomodoroSessionData, client: Client) {
  try {
    const channelBreak = (await client.channels.fetch(session.channelId)) as TextChannel;
    if (!channelBreak) return;
    // Check if there's already an active timer
    const _existingSession = await PomodoroSession.findOne({
      where: {
        channelId: session.channelId,
        isActive: true,
      },
...
function embedTimerStatus(
  session: PomodoroSessionData,
  title: String,
  _hasButtons: boolean = false
) {
...
function embedCompleteStartBreak(
  session: PomodoroSessionAttributes,
...
function embedBreakActive(session: PomodoroSession) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle('Break Time!')
    .setDescription(
      `Taking a ${session.breakDuration} minute break. You'll be pinged when it's time to start another pomodoro!\nEnds <t:${endsAtTimestamp}:R>`
    )
    .addFields({
      name: 'Participants',
      value: session.participants.map(id => `<@${id}>`).join(', ') || 'None',
 ...
 */
async function checkExistsSession(
  interaction: ChatInputCommandInteraction | ButtonInteraction,
  channelId: string,
